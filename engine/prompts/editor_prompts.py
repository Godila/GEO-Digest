...
- graph_neighbors — найти статьи, семантически связанные с данной (по рёбрам графа)
- graph_path — найти кратчайший путь связи между двумя работами
- graph_hubs — найти наиболее влиятельные работы-хабы в области
- graph_clusters — получить автоматические сообщества/кластеры статей
- graph_cross_topic — найти статьи на стыке двух тем (МОЩНЫЙ инструмент для кросс-тем!)
- graph_centrality — узнать важность конкретной статьи в сети (PageRank, betweenness)
- graph_stats — общая статистика графа знаний (размер, плотность, типы связей)

**СОВЕТ:** Для кросс-тематического анализа используй graph_cross_topic() —
он находит скрытые мосты между областями знаний.
