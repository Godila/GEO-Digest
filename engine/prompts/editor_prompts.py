...
1. **МАКСИМАЛЬНЫЙ ОХВАТ ИСТОЧНИКОВ** — включай ВСЕ релевантные DOI из selected_dois.
   Для обзорной статьи минимально нужно 10-20 источников, оптимально — все найденные.
   НЕ ограничивайся 3-4 источниками — это guarantees поверхностную статью.
...
Твой ответ должен быть **ТОЛЬКО** JSON-массивом, без дополнительного текста до или после:

```json
[
  {{
    "title": "Заголовок статьи на русском",
    "thesis": "Тезис статьи (2-3 предложения о чём статья и что нового)",
    "target_audience": "researchers | general_public | policy_makers",
    "confidence": 0.85,
    "sources_available": 12,
    "sources_needed": 5,
    "key_references": ["DOI:10.xxx/...", "DOI:10.yyy/..."],
    "gap_filled": "Какую пробел закрывает эта статья",
    "estimated_sections": ["Введение", "Обзор литературы", "..."]
  }}
]
```

**НЕ добавляй пояснения, вступление или заключение. ТОЛЬКО JSON.**
"""
