...
SECTION_TARGETS = {
    "введение":        {"words": (900, 1800),   "tokens": 15000},
    "обзор литературы": {"words": (3000, 5250),  "tokens": 36000},
    "методы":           {"words": (2250, 3750),  "tokens": 27000},
    "результаты":       {"words": (2700, 4500),  "tokens": 36000},
    "обсуждение":       {"words": (3000, 5250),  "tokens": 36000},
    "выводы":           {"words": (600, 1050),   "tokens": 10500},
    "список литературы": {"words": (300, 750),   "tokens": 6000},
    # English fallback
    "introduction":    {"words": (900, 1800),   "tokens": 15000},
    "literature":      {"words": (3000, 5250),  "tokens": 36000},
    "methods":         {"words": (2250, 3750),  "tokens": 27000},
    "results":         {"words": (2700, 4500),  "tokens": 36000},
    "discussion":      {"words": (3000, 5250),  "tokens": 36000},
    "conclusion":      {"words": (600, 1050),   "tokens": 10500},
    "references":      {"words": (300, 750),    "tokens": 6000},
}
...
# ═══════════════════════════════════════════════════════════════
#  PASS 2c: EXPAND SHORT SECTION — расширение короткой секции
# ═══════════════════════════════════════════════════════════════

def build_expand_short_section_prompt(
    heading: str,
    current_text: str,
    target_words: int,
    evidence_context: str = "",
) -> dict[str, str]:
    """Build system + user prompts for expanding a section that came out too short.
    ...
    if evidence_context:
        context_block = f"""
== КОНТЕКСТ ИЗ ИСТОЧНИКОВ (конкретные данные, цитаты авторов, сравнения) ==
{evidence_context[:10000]}

ВАЖНО: Извлекай из контекста конкретные числа, названия методов, имена авторов, годы публикаций.
Используй эти данные для новых абзацев."""

    system = f"""Ты — научный писатель. Расширяешь секцию «{heading}» с {current_words} до {target_words} слов.

== ВАЖНЫЕ ПРАВИЛА РАСШИРЕНИЯ ==
1. СОХРАНИ весь существующий текст ПОЛНОСТЬЮ — слово в слово.
2. ДОБАВЬ новые абзацы МЕЖДУ существующими или В КОНЦЕ секции.
3. НЕ сокращай и НЕ перефразируй существующие предложения.
4. Каждый новый абзац — 4-8 предложений с конкретными данными.

== ЧТО ДОБАВИТЬ В НОВЫХ АБЗАЦАХ ==
- Конкретные числовые данные (концентрации, проценты, координаты, магнитуды)
- Сравнения с другими исследованиями: [Автор и др., год] → наши результаты → различия
- Примеры и case studies из литературы
- Прямые цитаты авторов: Как показал [Author, Year], «...»
- Дополнительный анализ: тренды, аномалии, зависимости
- Ограничения и перспективы (если применимо к данной секции)
...
== ФОРМАТ ВЫВОДА ==
Верни JSON (без markdown-обёрток ```json ```):
{{"heading": "{heading}", "content": "расширенный текст секции с сохранённым оригиналом + новые абзацы"}}"""

    user = f"""== ТЕКУЩИЙ ТЕКСТ СЕКЦИИ «{heading}» ({current_words} слов) ==
{current_text}
{context_block}

== ЗАДАЧА ==
Расширь текст секции «{heading}» до {target_words} слов.
СОХРАНИ весь существующий текст. ДОБАВЬ содержательные абзацы с конкретными данными, сравнениями, цитатами авторов.
Каждый новый абзац — 4-8 предложений."""
...
КРИТИЧЕСКИЕ ПРАВИЛА ОБЪЁМА:
1. КАЖДАЯ переписанная секция должна быть НЕ КОРОЧЕ оригинала
2. Если оригинал секции 800 слов — переписанная версия минимум 800 слов
3. Если нужно что-то убрать — заменяй на более глубокий анализ, а не удаляй
4. ИТОГОВЫЙ ОБЩИЙ ОБЪЁМ СТАТЬИ НЕ ДОЛЖЕН УМЕНЬШИТЬСЯ
...
3. Добавляй новые аргументы, сравнения, примеры — но НЕ удаляй существующие
4. Обсуждение: минимум 1000 слов (сравнение с другими исследованиями, механизмы, ограничения)
5. Результаты: минимум 800 слов (конкретные цифры, таблицы сравнений)
6. Выводы: минимум 400 слов (резюме каждого ключевого вывода, значимость, перспективы)
7. Сохраняй академический стиль без AI-штампов
...
Каждая секция должна быть содержательной и глубокой:
- Введение: 6-12 абзацев, 900-1800 слов (контекст → история → проблема → цель → задачи)
- Обзор литературы: 12-22 абзацев, 3000-5250 слов (критический анализ, сравнение подходов, выявление пробелов)
- Методы: 10-16 абзацев, 2250-3750 слов (данные → методы → обоснование → параметры → ограничения)
- Результаты: 12-19 абзацев, 2700-4500 слов (цифры → таблицы → сравнения → статистика)
- Обсуждение: 12-22 абзацев, 3000-5250 слов (интерпретация → сравнение → механизм → ограничения → перспективы)
- Выводы: 3-5 абзацев, 600-1050 слов (резюме + значимость + рекомендации)"""
...
def build_max_tokens(group_type: GroupType) -> int:
    ...
    return min(int(target["target"] * 1.5 * 1.5), 64000)
